/* beammpc.f -- translated by f2c (version 20200916).
...
/* Subroutine */ int beammpc_(integer *ipompc, integer *nodempc, char *labmpc,
	 integer *nmpc, integer *nmpc___, integer *mpcfree, integer *ikmpc, 
	integer *ilmpc, integer *nk, integer *nk___, integer *nodeboun, 
	integer *ndirboun, integer *ikboun, integer *ilboun, integer *nboun, 
	integer *nboun___, integer *inode, integer *node, doublereal *co, 
	char *typeboun, ftnlen labmpc_len, ftnlen typeboun_len)
{
    ...
    integer j, k, l, m;
    doublereal dd;
    integer id, mpcfreeold, idof;
    doublereal dmax__;
    integer jmax;
    extern /* Subroutine */ int exit_(integer *);
    static integer nodea;
    integer nodeb;
...
/*     is node b. */
	...
	dmax__ = 0.;
	for (j = 1; j <= 3; ++j) {
	    dd = (d__1 = co[j + nodea * 3] - co[j + nodeb * 3], abs(d__1));
	    if (dd > dmax__) {
		dmax__ = dd;
		jmax = j;
	    }
    ...
    if (*nk > *nk___) {
	s_wsle(&io___7);
	do_lio(&c__9, &c__1, "*ERROR in beammpc: increase nk_", (ftnlen)31);
	e_wsle();
	exit_(&c__201);
    }
	...
	if (ikmpc[id] == idof) {
	    s_wsle(&io___12);
	    do_lio(&c__9, &c__1, "*WARNING in beammpc: DOF for node ", (
		    ftnlen)34);
    ...
    if (*nmpc > *nmpc___) {
	s_wsle(&io___16);
	do_lio(&c__9, &c__1, "*ERROR in beammpc: increase nmpc_", (ftnlen)33);
	e_wsle();
	exit_(&c__201);
    }

    ipompc[*nmpc] = *mpcfree;
    s_copy(labmpc + *nmpc * 20, "BEAM                ", (ftnlen)20, (ftnlen)
	    20);
    ...
    *mpcfree = nodempc[*mpcfree * 3 + 3];
    ...
    if (*nboun > *nboun___) {
	s_wsle(&io___19);
	do_lio(&c__9, &c__1, "*ERROR in beammpc: increase nboun_", (ftnlen)34)
		;
	e_wsle();
	exit_(&c__201);
    }
