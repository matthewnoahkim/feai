/* objective_freq_cs.f -- translated by f2c (version 20200916).
...
/* Subroutine */ int objective_freq_cs__(doublereal *dgdx, doublereal *df, 
	doublereal *vold, integer *ndesi, integer *iobject, integer *mi, 
	integer *nactdofinv, integer *jqs, integer *irows, integer *nk, 
	integer *nzss)
{
...
/*     ---------------------------------------------------------------- */
	...
	for (j = jqs[idesvar]; j <= i__2; ++j) {
	    ...
	    dgdx[idesvar + *iobject * dgdx_dim1] = dgdx[idesvar + *iobject * 
		    dgdx_dim1] - vold[idir + node * vold_dim1] * df[j] - vold[
		    idir + (*nk + node) * vold_dim1] * df[*nzss + j];
	}
    }
