/* writeturdir.f -- translated by f2c (version 20200916).
...
/* Subroutine */ int writeturdir_(doublereal *xn, char *turdir, integer *nev, 
	ftnlen turdir_len)
{
    /* Format strings */
...
/*     writes the eigenvalues in the .dat file */
    ...
    e_wsle();
    s_wsle(&io___7);
    do_lio(&c__9, &c__1, "MODE NO     TURNING DIRECTION (F=FORWARD,B=BACKWAR"
	    "D)", (ftnlen)52);
    e_wsle();
    s_wsle(&io___8);
    e_wsle();
