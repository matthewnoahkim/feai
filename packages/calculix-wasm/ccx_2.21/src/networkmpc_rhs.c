/* networkmpc_rhs.f -- translated by f2c (version 20200916).
...
/* Subroutine */ int networkmpc_rhs__(integer *i__, integer *ipompc, integer *
	nodempc, doublereal *coefmpc, char *labmpc, doublereal *v, doublereal 
	*bc, integer *j, integer *mi, integer *ipkon, integer *kon, char *
	...
	*prop, integer *ielmat, doublereal *shcon, integer *nshcon, 
	...
	cocon, integer *ncocon, ftnlen labmpc_len, ftnlen lakon_len)
{
    /* System generated locals */
    integer ielmat_dim1, ielmat_offset, v_dim1, v_offset, shcon_dim2, 
...
/*        to be calculated: bc(j):=-f */
	...
	bc[*j] = coefmpc[index] * v[idir + node * v_dim1];

	index = nodempc[index * 3 + 3];
	node = nodempc[index * 3 + 1];
	idir = nodempc[index * 3 + 2];
/* Computing 2nd power */
	d__1 = v[idir + node * v_dim1];
	bc[*j] += coefmpc[index] * (d__1 * d__1);

	bc[*j] = -bc[*j];
    } else {
	s_wsle(&io___4);
	do_lio(&c__9, &c__1, "*ERROR in networkmpc_rhs:", (ftnlen)25);
	e_wsle();
	s_wsle(&io___5);
