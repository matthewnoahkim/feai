/* inputerror.f -- translated by f2c (version 20200916).
...
/* Subroutine */ int inputerror_(char *inpc, integer *ipoinpc, integer *iline,
	 char *text, integer *ier, ftnlen inpc_len, ftnlen text_len)
{
...
/*     input error message subroutine */
    ...
    /* Function Body */
    s_wsle(&io___1);
    do_lio(&c__9, &c__1, "*ERROR reading ", (ftnlen)15);
    do_lio(&c__9, &c__1, text, i_indx(text, "%", text_len, (ftnlen)1) - 1);
    do_lio(&c__9, &c__1, ". Card image:", (ftnlen)13);
    e_wsle();
    ci__1.cierr = 0;
    ci__1.ciunit = 6;
    ci__1.cifmt = "(8x,1320a1)";
    s_wsfe(&ci__1);
    i__1 = ipoinpc[*iline];
