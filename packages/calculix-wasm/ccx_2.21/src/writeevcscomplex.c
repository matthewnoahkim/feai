/* writeevcscomplex.f -- translated by f2c (version 20200916).
...
/* Subroutine */ int writeevcscomplex_(doublecomplex *x, integer *nx, integer 
	*nm, doublereal *fmin, doublereal *fmax)
{
...
/*     nm is the nodal diameter */
    ...
    e_wsle();
    s_wsle(&io___5);
    do_lio(&c__9, &c__1, " NODAL   MODE NO                           FREQUEN"
	    "CY", (ftnlen)52);
    e_wsle();
    s_wsle(&io___6);
    do_lio(&c__9, &c__1, "DIAMETER                     REAL PART            "
	    "      IMAGINARY PART", (ftnlen)70);
    e_wsle();
    s_wsle(&io___7);
    do_lio(&c__9, &c__1, "                    (RAD/TIME)      (CYCLES/TIME) "
	    "       (RAD/TIME)", (ftnlen)67);
    e_wsle();
    s_wsle(&io___8);
    e_wsle();
	...
	ci__1.cierr = 0;
	ci__1.ciunit = 5;
	ci__1.cifmt = "(i5,4x,i7,3(2x,e14.7))";
	s_wsfe(&ci__1);
	do_fio(&c__1, (char *)&nm[j], (ftnlen)sizeof(integer));
