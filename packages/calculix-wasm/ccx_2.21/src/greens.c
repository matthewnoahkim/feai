/* greens.f -- translated by f2c (version 20200916).
...
/* Subroutine */ int greens_(char *inpc, char *textpart, integer *nmethod, 
	integer *mei, integer *iperturb, integer *istep, integer *istat, 
	integer *n, integer *iline, integer *ipol, integer *inl, integer *
	ipoinp, integer *inp, integer *ithermal, integer *isolver, doublereal 
	*xboun, integer *nboun, integer *ipoinpc, integer *ier, ftnlen 
	inpc_len, ftnlen textpart_len)
{
...
/*     reading the input deck: *GREEN */
    ...
    if (*istep < 1) {
	s_wsle(&io___1);
	do_lio(&c__9, &c__1, "*ERROR reading *GREEN: *GREEN can only be used",
		 (ftnlen)46);
	e_wsle();
	s_wsle(&io___2);
...
/*     default solver */
	...
	if (s_cmp(textpart + i__ * 132, "SOLVER=", (ftnlen)7, (ftnlen)7) == 0)
	    ...
	    s_wsle(&io___5);
	    do_lio(&c__9, &c__1, "*WARNING reading *GREEN: parameter not rec"
		    "ognized:", (ftnlen)50);
	    e_wsle();
	    s_wsle(&io___6);
	    ...
	    e_wsle();
	    inputwarning_(inpc + 1, ipoinpc, iline, "*GREEN%", (ftnlen)1, (
		    ftnlen)7);
	}
    }
	...
	*isolver = 0;
	     ...
	     {
	s_wsle(&io___7);
	do_lio(&c__9, &c__1, "*WARNING reading *GREEN: the iterative scaling",
		 (ftnlen)46);
	e_wsle();
	s_wsle(&io___8);
	do_lio(&c__9, &c__1, "         procedure is not available for green", 
		(ftnlen)45);
	e_wsle();
	s_wsle(&io___9);
	...
	*isolver = 8;
	...
	s_wsle(&io___13);
	do_lio(&c__9, &c__1, "*WARNING reading *GREEN: unknown solver;", (
		ftnlen)40);
	e_wsle();
	s_wsle(&io___14);
	...
	s_wsle(&io___15);
	do_lio(&c__9, &c__1, "*ERROR reading *GREEN: the default solver ", (
		ftnlen)42);
	do_lio(&c__9, &c__1, solver, (ftnlen)20);
	e_wsle();
	s_wsle(&io___16);
	do_lio(&c__9, &c__1, "       cannot be used for green calculations ", 
		(ftnlen)45);
	e_wsle();
	*ier = 1;
	return 0;
