/* extrapolateshell_us3.f -- translated by f2c (version 20200916).
...
/* Subroutine */ int extrapolateshell_us3__(doublereal *yi, doublereal *yn, 
	...
	iorienloc, char *cflag, integer *ielmat, doublereal *thicke, integer *
	ielprop, doublereal *prop, integer *i__, integer *iflag, ftnlen 
	lakon_len, ftnlen cflag_len)
{
    /* System generated locals */
    integer ielorien_dim1, ielorien_offset, ielmat_dim1, ielmat_offset, 
	    yi_dim1, yi_dim2, yi_offset, yn_dim1, yn_offset, thicke_dim1, 
	    thicke_offset, i__1;
...
/*     START OF THIS SUBROUTINE */
    ...
    if (*iorienloc != 0) {
	s_wsle(&io___1);
	do_lio(&c__9, &c__1, "*ERROR in extrapolate_us3", (ftnlen)25);
	e_wsle();
	s_wsle(&io___2);
...
/* MID */
		...
		if (*iflag == 0) {
/* gauss point 2 */
		    yn[k + node * yn_dim1] += yi[k + (*i__ * yi_dim2 + 2) * 
			    yi_dim1];
		}
...
/* TOP */
		...
		if (*iflag == 1) {
/* gauss point 3 */
		    yn[k + node * yn_dim1] += yi[k + (*i__ * yi_dim2 + 3) * 
			    yi_dim1];
		}
	    }
	    ++inum[node];
/* add divider */
	...
	e_wsle();
	s_wsle(&io___10);
	do_lio(&c__9, &c__1, "       extropolation for element of type US3", (
		ftnlen)44);
	e_wsle();
	s_wsle(&io___11);
