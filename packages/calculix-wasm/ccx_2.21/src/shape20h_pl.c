/* shape20h_pl.f -- translated by f2c (version 20200916).
...
/* Subroutine */ int shape20h_pl__(doublereal *xi, doublereal *et, doublereal 
...
/*      enddo */
    for (j = 1; j <= 3; ++j) {
	xs[j * 3 - 3] = xl[4] * (shpe[j - 1] + shpe[j + 15] + shpe[j + 63]) + 
		xl[7] * (shpe[j + 3] + shpe[j + 19] + shpe[j + 67]) + xl[10] *
		 (shpe[j + 7] + shpe[j + 23] + shpe[j + 71]) + xl[13] * (shpe[
		j + 11] + shpe[j + 27] + shpe[j + 75]) + xl[28] * (shpe[j + 
