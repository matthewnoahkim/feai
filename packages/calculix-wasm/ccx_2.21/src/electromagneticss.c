/* electromagneticss.f -- translated by f2c (version 20200916).
...
/* Subroutine */ int electromagneticss_(char *inpc, char *textpart, integer *
	...
	*istat, integer *n, doublereal *tinc, doublereal *tper, doublereal *
	...
	doublereal *alpha, integer *mei, doublereal *fei, integer *ipoinpc, 
	doublereal *ctrl, doublereal *ttime, integer *ier, ftnlen inpc_len, 
	ftnlen textpart_len)
{
...
/*     defaults for fmin and fmax */
	...
	s_wsle(&io___4);
	do_lio(&c__9, &c__1, "*ERROR reading *ELECTROMAGNETICS: perturbation"
		" analysis is", (ftnlen)58);
	e_wsle();
	s_wsle(&io___5);
	do_lio(&c__9, &c__1, "       not provided in a *ELECTROMAGNETICS ste"
		"p.", (ftnlen)48);
	e_wsle();
	*ier = 1;
	return 0;
    ...
    if (*istep < 1) {
	s_wsle(&io___6);
	do_lio(&c__9, &c__1, "*ERROR reading *ELECTROMAGNETICS: *ELECTROMAGN"
		"ETICS can only", (ftnlen)60);
	e_wsle();
	s_wsle(&io___7);
...
/*     default solver */
	...
	if (s_cmp(textpart + i__ * 132, "SOLVER=", (ftnlen)7, (ftnlen)7) == 0)
		...
		ftnlen)9) == 0) {
	    idirect = 0;
	} else if (s_cmp(textpart + i__ * 132, "MAGNETOSTATICS", (ftnlen)14, (
		ftnlen)14) == 0) {
	    *nmethod = 8;
	} else if (s_cmp(textpart + i__ * 132, "FREQUENCY", (ftnlen)9, (
		ftnlen)9) == 0) {
	    *nmethod = 10;
	    iheat = 0;
...
L100002:
	    ;
	} else if (s_cmp(textpart + i__ * 132, "NOHEATTRANSFER", (ftnlen)14, (
		ftnlen)14) == 0) {
	    iheat = 0;
	} else if (s_cmp(textpart + i__ * 132, "OMEGA=", (ftnlen)6, (ftnlen)6)
		 == 0) {
	    ici__1.icierr = 1;
	    ici__1.iciend = 1;
	    ici__1.icirnum = 1;
...
L100003:
	    ...
	    s_wsle(&io___12);
	    do_lio(&c__9, &c__1, "*WARNING reading *ELECTROMAGNETICS: parame"
	    ...
	    e_wsle();
	    inputwarning_(inpc + 1, ipoinpc, iline, "*ELECTROMAGNETICS%", (
		    ftnlen)1, (ftnlen)18);
	}
    }
    if (*nmethod == 8) {
...
/*     for static or dynamic calculations DIRECT=NO */
	...
	*idrct = 0;
	...
	if (iheat == 1) {
	    if (ithermal[1] == 0) {
		s_wsle(&io___14);
		do_lio(&c__9, &c__1, "*ERROR reading *ELECTROMAGNETICS: plea"
			"se define initial ", (ftnlen)56);
		e_wsle();
		s_wsle(&io___15);
		do_lio(&c__9, &c__1, "       conditions for the temperature", 
			(ftnlen)37);
		e_wsle();
		*ier = 1;
		return 0;
    ...
    if (*nmethod != 10 && iperturb[1] != 0) {

/*        static or dynamic thermal analysis */

	if (s_cmp(solver, "SPOOLES", (ftnlen)7, (ftnlen)7) == 0) {
	    *isolver = 0;
	} else if (s_cmp(solver, "ITERATIVESCALING", (ftnlen)16, (ftnlen)16) 
		== 0) {
	    *isolver = 2;
	    ...
	    *isolver = 8;
	    ...
	    s_wsle(&io___16);
	    do_lio(&c__9, &c__1, "*WARNING reading *ELECTROMAGNETICS: unknow"
		    "n solver;", (ftnlen)51);
	    e_wsle();
	    s_wsle(&io___17);
	    do_lio(&c__9, &c__1, "         the default solver is used", (
		    ftnlen)35);
	    e_wsle();
	}
	...
	if (*istat < 0 || key == 1) {
	    if (iperturb[1] >= 2) {
		s_wsle(&io___19);
		do_lio(&c__9, &c__1, "*WARNING reading *ELECTROMAGNETICS: a "
			"nonlinear geometric analysis is requested", (ftnlen)
			79);
		e_wsle();
		s_wsle(&io___20);
		do_lio(&c__9, &c__1, "         but no time increment nor ste"
			"p is specified", (ftnlen)52);
		e_wsle();
		s_wsle(&io___21);
...
L100004:
	if (*istat > 0) {
	    inputerror_(inpc + 1, ipoinpc, iline, "*ELECTROMAGNETICS%", ier, (
		    ftnlen)1, (ftnlen)18);
	    return 0;
	}
	ici__1.icierr = 1;
...
L100007:
	...
	if (*tinc <= 0.) {
	    s_wsle(&io___22);
	    do_lio(&c__9, &c__1, "*ERROR reading *ELECTROMAGNETICS: initial "
		    "increment size is negative", (ftnlen)68);
	    e_wsle();
	}
	if (*tper <= 0.) {
	    s_wsle(&io___23);
	    do_lio(&c__9, &c__1, "*ERROR reading *ELECTROMAGNETICS: step siz"
		    "e is negative", (ftnlen)55);
	    e_wsle();
	}
	if (*tinc > *tper) {
	    s_wsle(&io___24);
	    do_lio(&c__9, &c__1, "*ERROR reading *ELECTROMAGNETICS: initial "
		    "increment size exceeds step size", (ftnlen)74);
	    e_wsle();
	}
...
/*        steady state electrodynamics analysis */
	...
	getnewline_(inpc + 1, textpart + 132, istat, n, &key, iline, ipol, 
		inl, &ipoinp[3], &inp[4], ipoinpc, (ftnlen)1, (ftnlen)132);
	s_wsle(&io___27);
	do_lio(&c__9, &c__1, "*INFO reading *ELECTROMAGNETICS:", (ftnlen)32);
	e_wsle();
	s_wsle(&io___28);
	do_lio(&c__9, &c__1, "      for a steady state electromagnetics calc"
		"ulation", (ftnlen)53);
	e_wsle();
	s_wsle(&io___29);
	do_lio(&c__9, &c__1, "      the time defaults (1,1) are used", (
		ftnlen)38);
	e_wsle();
	*tinc = 1.;
	*tper = 1.;
