/* materialdata_dvi.f -- translated by f2c (version 20200916).
...
/* Subroutine */ int materialdata_dvi__(doublereal *shcon, integer *nshcon, 
	integer *imat, doublereal *dvi, doublereal *t1l, integer *ntmat___, 
	integer *itherm)
{
	...
	*dvi = shcon[(*imat * shcon_dim2 + 1 << 2) + 2];
	...
	if (nshcon[*imat] == 0) {
	    ...
	    *dvi = shcon[(id + *imat * shcon_dim2 << 2) + 2] + (shcon[(id + 1 
		    + *imat * shcon_dim2 << 2) + 2] - shcon[(id + *imat * 
		    shcon_dim2 << 2) + 2]) * (*t1l - shcon[(id + *imat * 
