/* dKdp.f -- translated by f2c (version 20200916).
...
/* Subroutine */ int dkdp_(doublereal *x, doublereal *u, doublereal *uprime, 
...
/*     df1dK */
    ...
    uprime[1] = pow_dd(x, &c_b7) * .8 * lambda1 * pow_dd(&pup, &c_b8) / (
	    ...
	    x, &c_b7) * pow_dd(&pup, &c_b9) / (xflow * pow_dd(&tup, &c_b9)) * 
	    df1dk - 2. / *x);
