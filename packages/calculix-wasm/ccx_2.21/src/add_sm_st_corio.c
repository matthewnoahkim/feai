/* add_sm_st_corio.f -- translated by f2c (version 20200916).
...
/* Subroutine */ int add_sm_st_corio__(doublereal *au, doublereal *ad, 
	integer *jq, integer *irow, integer *i__, integer *j, doublereal *
	value, integer *i0, integer *i1)
{
...
/*         endif */
    ...
    if (irow[ipointer] != ii) {
	s_wsle(&io___6);
	do_lio(&c__9, &c__1, "*ERROR in add_sm_st: coefficient should be 0", (
		ftnlen)44);
	e_wsle();
	exit_(&c__201);
    } else {
	au[ipointer] += valuenew;
    }
