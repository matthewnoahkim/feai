/* uncouptempdisps.f -- translated by f2c (version 20200916).
...
/* Subroutine */ int uncouptempdisps_(char *inpc, char *textpart, integer *
	...
	*istat, integer *n, doublereal *tinc, doublereal *tper, doublereal *
	...
	integer *ipoinpc, doublereal *alpha, doublereal *ctrl, doublereal *
	ttime, integer *nener, integer *ier, ftnlen inpc_len, ftnlen 
	textpart_len)
{
...
/*             8: pastix */
    ...
    *nmethod = 4;
	...
	s_wsle(&io___2);
	do_lio(&c__9, &c__1, "*ERROR reading *UNCOUPLED TEMPERATURE-DISPLACE"
		"MENT:", (ftnlen)51);
	e_wsle();
	s_wsle(&io___3);
	...
	e_wsle();
	s_wsle(&io___4);
	do_lio(&c__9, &c__1, "       *UNCOUPLED TEMPERATURE-DISPLACEMENT ste"
		"p.", (ftnlen)48);
	e_wsle();
	*ier = 1;
	return 0;
	...
	e_wsle();
	s_wsle(&io___6);
	do_lio(&c__9, &c__1, "       *UNCOUPLED TEMPERATURE-DISPLACMENT ", (
		ftnlen)42);
	e_wsle();
	s_wsle(&io___7);
	do_lio(&c__9, &c__1, "       can only be used within a STEP", (ftnlen)
		37);
	e_wsle();
	*ier = 1;
	return 0;
...
L100001:
	    if (*istat > 0) {
		inputerror_(inpc + 1, ipoinpc, iline, "*UNCOUPLED TEMPERATUR"
			"E-DISPLACEMENT%", ier, (ftnlen)1, (ftnlen)36);
		return 0;
	    }
...
L100003:
	    ...
	    s_wsle(&io___14);
	    do_lio(&c__9, &c__1, "*WARNING in uncouptempdisps: parameter not"
		    " recognized:", (ftnlen)54);
	    e_wsle();
	    s_wsle(&io___15);
	    ...
	    e_wsle();
	    inputwarning_(inpc + 1, ipoinpc, iline, "*UNCOUPLED TEMPERATURE-"
		    "DISPLACEMENT%", (ftnlen)1, (ftnlen)36);
	}
    }
    if (*nmethod == 1) {
	ctrl[27] = 1e30;
	...
	*isolver = 8;
	...
	s_wsle(&io___19);
	do_lio(&c__9, &c__1, "*WARNING reading *UNCOUPLED TEMPERATURE-DISPLA"
		"CEMENT:", (ftnlen)53);
	e_wsle();
	s_wsle(&io___20);
	...
	if (iperturb[1] >= 2) {
	    s_wsle(&io___23);
	    do_lio(&c__9, &c__1, "*WARNING reading *UNCOUPLED TEMPERATURE-DI"
		    "SPLACEMENT:", (ftnlen)53);
	    e_wsle();
	    s_wsle(&io___24);
	    do_lio(&c__9, &c__1, "         a nonlinear analysis is requested",
		     (ftnlen)42);
	    e_wsle();
	    s_wsle(&io___25);
...
L100004:
    if (*istat > 0) {
	inputerror_(inpc + 1, ipoinpc, iline, "*UNCOUPLED TEMPERATURE-DISPLA"
		"CEMENT%", ier, (ftnlen)1, (ftnlen)36);
	return 0;
    }
    ici__1.icierr = 1;
    ici__1.iciend = 1;
...
L100007:
	...
	e_wsle();
	s_wsle(&io___28);
	do_lio(&c__9, &c__1, "       initial increment size is              "
		"     negative", (ftnlen)59);
	e_wsle();
    }
    if (*tper <= 0.) {
	...
	e_wsle();
	s_wsle(&io___32);
	do_lio(&c__9, &c__1, "       initial increment size exc             "
		"     eeds step size", (ftnlen)65);
	e_wsle();
    }
