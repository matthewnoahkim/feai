/* cfds.f -- translated by f2c (version 20200916).
...
/* Subroutine */ int cfds_(char *inpc, char *textpart, integer *nmethod, 
	integer *iperturb, integer *isolver, integer *istep, integer *istat, 
	integer *n, doublereal *tinc, doublereal *tper, doublereal *tmin, 
	doublereal *tmax, integer *idrct, integer *ithermal, integer *iline, 
	integer *ipol, integer *inl, integer *ipoinp, integer *inp, integer *
	ipoinpc, doublereal *alpha, doublereal *ctrl, integer *iexpl, 
	doublereal *tincf, doublereal *ttime, doublereal *physcon, integer *
	ier, ftnlen inpc_len, ftnlen textpart_len)
{
...
/*     FVM method: 2D/3D switch: default 3D */
	...
	s_wsle(&io___4);
	do_lio(&c__9, &c__1, "*ERROR reading *CFD: perturbation analysis is", 
		(ftnlen)45);
	e_wsle();
	s_wsle(&io___5);
	do_lio(&c__9, &c__1, "       not provided in a *HEAT TRANSFER step.", 
		(ftnlen)45);
	e_wsle();
	*ier = 1;
	return 0;
    ...
    if (*istep < 1) {
	s_wsle(&io___6);
	do_lio(&c__9, &c__1, "*ERROR reading *CFD: *HEAT TRANSFER can only ", 
		(ftnlen)45);
	e_wsle();
	s_wsle(&io___7);
...
/*     default solver */
	...
	if (s_cmp(textpart + i__ * 132, "SOLVER=", (ftnlen)7, (ftnlen)7) == 0)
		...
		ftnlen)12) == 0) {
	    *iexpl = 1;
	} else if (s_cmp(textpart + i__ * 132, "SHALLOWWATER", (ftnlen)12, (
		ftnlen)12) == 0) {
	    shallowwater = TRUE_;
	    *iexpl = 1;
...
L100001:
	    ;
	} else if (s_cmp(textpart + i__ * 132, "TURBULENCEMODEL=", (ftnlen)16,
		 (ftnlen)16) == 0) {

/*     turbulence model */

	    if (s_cmp(textpart + (i__ * 132 + 16), "NONE", (ftnlen)9, (ftnlen)
		    4) == 0) {
		physcon[9] = .5;
	    } else if (s_cmp(textpart + (i__ * 132 + 16), "K-EPSILON", (
		    ftnlen)9, (ftnlen)9) == 0) {
		physcon[9] = 1.5;
	    } else if (s_cmp(textpart + (i__ * 132 + 16), "K-OMEGA", (ftnlen)
		    7, (ftnlen)7) == 0) {
		physcon[9] = 2.5;
	    } else if (s_cmp(textpart + (i__ * 132 + 16), "BSL", (ftnlen)3, (
		    ftnlen)3) == 0) {
		physcon[9] = 3.5;
	    } else if (s_cmp(textpart + (i__ * 132 + 16), "SST", (ftnlen)3, (
		    ftnlen)3) == 0) {
		physcon[9] = 4.5;
	    }
	} else if (s_cmp(textpart + i__ * 132, "2D", (ftnlen)2, (ftnlen)2) == 
		0) {
	    physcon[10] = 2.5;
	} else if (s_cmp(textpart + i__ * 132, "SCHEME=UD", (ftnlen)9, (
		ftnlen)9) == 0) {
	    ctrl[48] = 1.5;
	} else if (s_cmp(textpart + i__ * 132, "SCHEME=MODSMART", (ftnlen)15, 
		(ftnlen)15) == 0) {
	    ctrl[48] = 2.5;
	} else if (s_cmp(textpart + i__ * 132, "SIMPLEC", (ftnlen)7, (ftnlen)
		7) == 0) {
	    ctrl[49] = 1.5;
	} else if (s_cmp(textpart + i__ * 132, "FEM", (ftnlen)3, (ftnlen)3) ==
		 0) {
	    fem = TRUE_;
	} else {
	    s_wsle(&io___10);
	    do_lio(&c__9, &c__1, "*WARNING reading *CFD: parameter not recog"
		    "nized:", (ftnlen)48);
	    e_wsle();
	    s_wsle(&io___11);
	    ...
	    e_wsle();
	    inputwarning_(inpc + 1, ipoinpc, iline, "*CFD%", (ftnlen)1, (
		    ftnlen)5);
	}
    }
    if (*nmethod == 1) {
    ...
    if (ithermal[1] == 0 && *iexpl == 1 && ! shallowwater) {
	s_wsle(&io___12);
	do_lio(&c__9, &c__1, "*ERROR reading *CFD: please define initial ", (
		ftnlen)43);
	e_wsle();
	s_wsle(&io___13);
	...
	*isolver = 8;
	...
	s_wsle(&io___14);
	do_lio(&c__9, &c__1, "*WARNING reading *CFD: unknown solver;", (
		ftnlen)38);
	e_wsle();
	s_wsle(&io___15);
...
/*     transient cfd */
	    ...
	    s_wsle(&io___17);
	    do_lio(&c__9, &c__1, "*WARNING reading *CFD: a nonlinear geometr"
		    "ic      analysis is requested", (ftnlen)71);
	    e_wsle();
	    s_wsle(&io___18);
	    do_lio(&c__9, &c__1, "         but no time increment nor step is"
		    " speci  fied", (ftnlen)54);
	    e_wsle();
	    s_wsle(&io___19);
...
L100002:
    if (*istat > 0) {
	inputerror_(inpc + 1, ipoinpc, iline, "*CFD%", ier, (ftnlen)1, (
		ftnlen)5);
	return 0;
    }
    ici__1.icierr = 1;
...
L100006:
    ...
    if (*tinc <= 0.) {
	s_wsle(&io___20);
	do_lio(&c__9, &c__1, "*ERROR reading *CFD: initial increment size is"
		"      negative", (ftnlen)60);
	e_wsle();
    }
    if (*tper <= 0.) {
	s_wsle(&io___21);
	do_lio(&c__9, &c__1, "*ERROR reading *CFD: step size is negative", (
		ftnlen)42);
	e_wsle();
    }
    if (*tinc > *tper) {
	s_wsle(&io___22);
	do_lio(&c__9, &c__1, "*ERROR reading *CFD: initial increment size ex"
		"c     eeds step size", (ftnlen)66);
	e_wsle();
    }
...
/* Computing MIN */
	...
	if (*tinc > abs(*tmax)) {
	    s_wsle(&io___23);
	    do_lio(&c__9, &c__1, "*WARNING reading *CFD:", (ftnlen)22);
	    e_wsle();
	    s_wsle(&io___24);
