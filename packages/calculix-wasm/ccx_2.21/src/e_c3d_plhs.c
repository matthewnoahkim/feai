/* e_c3d_plhs.f -- translated by f2c (version 20200916).
...
/* Subroutine */ int e_c3d_plhs__(char *lakonl, doublereal *sm, integer *
...
/*     calculations) */
		...
		sm[i__ + (j << 3)] += (shp[(i__ << 2) - 4] * shp[(j << 2) - 4]
			 + shp[(i__ << 2) - 3] * shp[(j << 2) - 3] + shp[(i__ 
			<< 2) - 2] * shp[(j << 2) - 2]) * weight;
	    }
	}
    }
