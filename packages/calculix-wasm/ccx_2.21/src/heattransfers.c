/* heattransfers.f -- translated by f2c (version 20200916).
...
/* Subroutine */ int heattransfers_(char *inpc, char *textpart, integer *
...
/*     defaults for fmin and fmax */
	...
	s_wsle(&io___4);
	do_lio(&c__9, &c__1, "*ERROR reading *HEAT TRANSFER: perturbation an"
		"alysis is", (ftnlen)55);
	e_wsle();
	s_wsle(&io___5);
	...
	*ier = 1;
    ...
    if (*istep < 1) {
	s_wsle(&io___6);
	do_lio(&c__9, &c__1, "*ERROR reading *HEAT TRANSFER: *HEAT TRANSFER "
		"can only", (ftnlen)54);
	e_wsle();
	s_wsle(&io___7);
...
/*     default solver */
	...
	if (s_cmp(textpart + i__ * 132, "SOLVER=", (ftnlen)7, (ftnlen)7) == 0)
	    ...
	    *nmethod = 1;
		...
		ftnlen)9) == 0) {
	    *nmethod = 2;
	} else if (s_cmp(textpart + i__ * 132, "MODALDYNAMIC", (ftnlen)12, (
		ftnlen)12) == 0) {
	    iperturb[1] = 0;
...
L100002:
	    ...
	    s_wsle(&io___11);
	    do_lio(&c__9, &c__1, "*WARNING reading *HEAT TRANSFER: parameter"
	    ...
	    e_wsle();
	    inputwarning_(inpc + 1, ipoinpc, iline, "*HEAT TRANSFER%", (
		    ftnlen)1, (ftnlen)15);
	}
    }
    if (*nmethod == 1) {
...
/*     for static or dynamic calculations DIRECT=NO */
	...
	*idrct = 0;
    ...
    if (ithermal[1] == 0 && *nmethod != 1 && *nmethod != 2 && iperturb[1] != 
	    0) {
	s_wsle(&io___13);
	do_lio(&c__9, &c__1, "*ERROR reading *HEAT TRANSFER: please define i"
		"nitial ", (ftnlen)53);
	e_wsle();
	s_wsle(&io___14);
...
/*        static or dynamic thermal analysis */
	...
	if (s_cmp(solver, "SPOOLES", (ftnlen)7, (ftnlen)7) == 0) {
	    ...
	    *isolver = 5;
	} else if (s_cmp(solver, "MATRIXSTORAGE", (ftnlen)13, (ftnlen)13) == 
		0) {
	    *isolver = 6;
	    ...
	    *isolver = 8;
	    ...
	    s_wsle(&io___15);
	    do_lio(&c__9, &c__1, "*WARNING reading *HEAT TRANSFER: unknown s"
		    "olver;", (ftnlen)48);
	    e_wsle();
	    s_wsle(&io___16);
	...
	if (*istat < 0 || key == 1) {
	    if (iperturb[1] >= 2) {
		s_wsle(&io___18);
		do_lio(&c__9, &c__1, "*WARNING reading *HEAT TRANSFER: a non"
			"linear geometric analysis is requested", (ftnlen)76);
		e_wsle();
		s_wsle(&io___19);
...
L100003:
	if (*istat > 0) {
	    inputerror_(inpc + 1, ipoinpc, iline, "*HEAT TRANSFER%", ier, (
		    ftnlen)1, (ftnlen)15);
	    return 0;
	}
	ici__1.icierr = 1;
...
L100006:
	...
	if (*tinc <= 0.) {
	    s_wsle(&io___22);
	    do_lio(&c__9, &c__1, "*ERROR reading *HEAT TRANSFER: initial inc"
		    "rement sizeis negative", (ftnlen)64);
	    e_wsle();
	}
	if (*tper <= 0.) {
	    s_wsle(&io___23);
	    do_lio(&c__9, &c__1, "*ERROR reading *HEAT TRANSFER: step size i"
...
/*        thermal eigenmode analysis */
	...
	if (*istat < 0 || key == 1) {
	    s_wsle(&io___25);
	    do_lio(&c__9, &c__1, "*ERROR reading *HEAT TRANSFER: definition "
		    "not complete", (ftnlen)54);
	    e_wsle();
	    s_wsle(&io___26);
...
L100007:
	...
	if (nev <= 0) {
	    s_wsle(&io___28);
	    do_lio(&c__9, &c__1, "*ERROR in frequencies: less than 1 eigenva"
		    "lue requested", (ftnlen)55);
	    e_wsle();
	    *ier = 1;
	    return 0;
	}
