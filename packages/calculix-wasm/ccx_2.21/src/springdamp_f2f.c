/* springdamp_f2f.f -- translated by f2c (version 20200916).
...
/* Subroutine */ int springdamp_f2f__(doublereal *xl, doublereal *elas, 
	doublereal *voldl, doublereal *s, integer *imat, doublereal *elcon, 
	integer *ncmat___, integer *ntmat___, integer *nope, char *lakonl, 
	integer *iperturb, doublereal *springarea, integer *nmethod, integer *
	mi, doublereal *reltime, integer *nasym, integer *jfaces, integer *
	igauss, doublereal *pslavsurf, doublereal *pmastsurf, doublereal *
	clearini, ftnlen lakonl_len)
{
    /* Initialized data */
    ...
    doublereal c3, al[3], et, pl[57]	/* was [3][19] */, xi, xk, xm[3], xn[
	    3], tu[171]	/* was [3][3][19] */, xs2[21]	/* was [3][7] */, 
	    stickslope, dal[171]	/* was [3][3][19] */, fpu[171]	/* 
	    was [3][3][19] */, xs2s[21]	/* was [3][7] */, dval[57]	/* 
	    was [3][19] */, shp2m[63]	/* was [7][9] */, shp2s[63]	/* 
	    was [7][9] */, xsj2s[3], clear;
    integer nopem, nopep, nopes;
    doublereal pproj[3], weight;
...
/*     ( d al / d u_k) */
    ...
    for (i__ = 1; i__ <= i__1; ++i__) {
	for (j = 1; j <= 3; ++j) {
	    dal[j + (j + i__ * 3) * 3 - 13] = -shp2m[i__ * 7 - 4];
	}
    }
    for (j = 1; j <= 3; ++j) {
	dal[j + (j + nopep * 3) * 3 - 13] = 1.;
    }
...
/*        auxiliary variable: (d clear d u_k)*||m|| */
	...
	for (i__ = 1; i__ <= 3; ++i__) {
	    dval[i__ + k * 3 - 4] = xn[0] * dal[(i__ + k * 3) * 3 - 12] + xn[
		    1] * dal[(i__ + k * 3) * 3 - 11] + xn[2] * dal[(i__ + k * 
		    3) * 3 - 10];
	}

    }
    for (i__ = 1; i__ <= 3; ++i__) {
	dval[i__ + nopep * 3 - 4] = xn[0] * dal[(i__ + nopep * 3) * 3 - 12] + 
		xn[1] * dal[(i__ + nopep * 3) * 3 - 11] + xn[2] * dal[(i__ + 
		nopep * 3) * 3 - 10];
    }
...
/*     derivatives of the forces w.r.t. the displacement vectors */
	...
	for (j = 1; j <= 3; ++j) {
	    for (i__ = 1; i__ <= 3; ++i__) {
		fpu[i__ + (j + k * 3) * 3 - 13] = -c3 * xn[i__ - 1] * dval[j 
			+ k * 3 - 4];
	    }
	}
    }
    for (j = 1; j <= 3; ++j) {
	for (i__ = 1; i__ <= 3; ++i__) {
	    fpu[i__ + (j + nopep * 3) * 3 - 13] = -c3 * xn[i__ - 1] * dval[j 
		    + nopep * 3 - 4];
	}
    }
...
/*     tangential damping */
	...
	stickslope = elcon[(*imat * elcon_dim2 + 1) * elcon_dim1 + 8] * elcon[
		(*imat * elcon_dim2 + 1) * elcon_dim1 + 5];
...
/*       (d al/d u_k).||m|| -> (d al^+/d u_k).||m|| */
	...
	for (k = 1; k <= i__1; ++k) {
	    for (i__ = 1; i__ <= 3; ++i__) {
		dval[i__ + k * 3 - 4] = xn[0] * dal[(i__ + k * 3) * 3 - 12] + 
			xn[1] * dal[(i__ + k * 3) * 3 - 11] + xn[2] * dal[(
			i__ + k * 3) * 3 - 10];
	    }
	}
	for (i__ = 1; i__ <= 3; ++i__) {
	    dval[i__ + nopep * 3 - 4] = xn[0] * dal[(i__ + nopep * 3) * 3 - 
		    12] + xn[1] * dal[(i__ + nopep * 3) * 3 - 11] + xn[2] * 
		    dal[(i__ + nopep * 3) * 3 - 10];
	}
	...
	for (k = 1; k <= i__1; ++k) {
		...
		for (i__ = 1; i__ <= 3; ++i__) {
		    tu[i__ + (j + k * 3) * 3 - 13] = dal[i__ + (j + k * 3) * 
			    3 - 13] - xn[i__ - 1] * dval[j + k * 3 - 4];
		}
	    }
	}
	for (j = 1; j <= 3; ++j) {
	    for (i__ = 1; i__ <= 3; ++i__) {
		tu[i__ + (j + nopep * 3) * 3 - 13] = dal[i__ + (j + nopep * 3)
			 * 3 - 13] - xn[i__ - 1] * dval[j + nopep * 3 - 4];
	    }
	}
...
/*        damping matrix */
	...
	for (j = 1; j <= 3; ++j) {
	    for (i__ = 1; i__ <= 3; ++i__) {
		fpu[i__ + (j + nopep * 3) * 3 - 13] += xk * tu[i__ + (j + 
			nopep * 3) * 3 - 13];
	    }
	}
    }
